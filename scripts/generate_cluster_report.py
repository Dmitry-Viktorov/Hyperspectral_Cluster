...
def _methodology_section(doc: Document) -> None:
    ...
    # GMM
    _bold_para(doc, "Gaussian Mixture Model (современный метод):")
    doc.add_paragraph(
        ...
        "веса компонент) оцениваются EM-алгоритмом (Dempster et al., 1977). "
        "Преимущества перед K-Means: (1) мягкая кластеризация — каждый пиксель "
        "имеет вероятности принадлежности ко всем кластерам; (2) эллипсоидальные "
        "кластеры — лучше моделирует вытянутые распределения спектров; "
        "(3) возможность оценки неопределённости классификации."
    )
